...
function generateIconPng(size) {
  ...
  function writeUint16(v) { buf[offset++] = (v >> 8) & 0xff; buf[offset++] = v & 0xff; }
...
function createIco(pngBuffers) {
  ...
  const icoSize = dataOffset + pngBuffers.reduce((sum, buf) => sum + buf.length, 0);
  const ico = Buffer.alloc(icoSize);
  let offset = 0;

  function writeUint16(v) { ico[offset++] = v & 0xff; ico[offset++] = (v >> 8) & 0xff; }
  function writeUint32(v) { ico[offset++] = v & 0xff; ico[offset++] = (v >> 8) & 0xff; ico[offset++] = (v >> 16) & 0xff; ico[offset++] = (v >> 24) & 0xff; }
  ...
  for (let i = 0; i < count; i++) {
    ...
    const size = Math.round(Math.sqrt(png.length));
...
const pngBuffers = icoSizes.map(size => generateIconPng(size));

const ico = createIco(pngBuffers);
fs.writeFileSync(path.join(buildDir, 'icon.ico'), ico);
console.log('icon.ico generated!');

fs.writeFileSync(path.join(buildDir, 'icon_256.png'), generateIconPng(256));
